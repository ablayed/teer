...
async function getMerchantCurrency(supabase: SupabaseServerClient, merchantAccountId: string) {
...
export const listCustomersAction = requireRole('owner', 'manager', 'agent')
  ...
  .action(async ({ ctx, parsedInput }) => {
    ...
    const [customersResult, currency] = await Promise.all([
      supabase.rpc('list_customer_reliability', {
      ...
      getMerchantCurrency(supabase, ctx.member.merchantAccountId),
    ]);
...
export const getCustomerAction = requireRole('owner', 'manager', 'agent')
  ...
  .action(async ({ ctx, parsedInput }) => {
    const supabase = asTypedSupabaseClient(ctx.supabase);
    const [customerResult, historyResult, currency] = await Promise.all([
