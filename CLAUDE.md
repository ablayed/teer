...
**(j) Ouvert, non causé par UX-DS-01 — investigué, pas corrigé.** `tests/e2e/lot-f2-purchase-lot-detail.spec.ts:591` (« la correction du transport fait disparaître la marge provisoire (avant/après réel) ») a échoué une fois, iphone-14/WebKit uniquement, run `33900592606` (job `101113782009`, 2026-09-04) : après `page.mouse.click(10, 10)` (clic extérieur censé fermer le tiroir mobile Vaul), `expect(panel).toHaveCount(0)` échoue — le panneau reste `[active]`, entièrement monté, capturé par la trace du run en échec. Cause non démontrée avec certitude (nécessiterait une trace runtime du calcul de snap-point de vaul, hors de portée sans navigateur interactif), mais faisceau suffisant pour écarter les changements du lot : `components/ui/detail-panel.tsx:146` a toujours son contenu en `overflow-y-auto` — le débordement scrolle en interne, l'empreinte du tiroir extérieur (et donc la zone de clic extérieur) n'en dépend pas, ce qui exclut qu'un gain de quelques pixels sur les boutons/champs touchés par le lot ait pu déplacer le point de clic hors de la zone de fermeture. Suspect le plus probable : `components/ui/drawer.tsx:9` (`shouldScaleBackground = true`, effet d'échelle iOS de vaul sur l'arrière-plan), catégorie de flake connue sur WebKit lors de la transition d'ouverture — cohérent avec le commentaire du test lui-même (déjà présent avant ce lot), qui documente cette fermeture comme instable au clic direct sur le bouton « Fermer » et utilise le clic extérieur comme contournement déjà éprouvé. Rejoué sur l'arbre identique (run `33904838708`, même SHA `6c5904b`) : vert. Ne pas modifier `shouldScaleBackground`/`drawer.tsx` sur la seule foi de cette note — root cause non confirmée, effet transverse à tous les tiroirs mobiles de l'app.
