...
DB setup: `supabase link --project-ref <ref>` → `supabase db push`. Migrations live in `supabase/migrations/` and are applied in order; there is no migration ORM. **Latest applied migration: `0039` (in prod, types regenerated).**
...
| **8** | AI assistant (metrics function-calling, read-only, RLS-scoped) | — | ⬜ À venir. |
