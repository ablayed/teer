...
> **Migration tracker attestation (2026-08-27, corrigée en Lot D0-bis/Phase F) :** état confirmé par le porteur, `supabase migration list --linked` : **`0001` → `0144` toutes déployées, Local = Remote.** Cette attestation corrige la précédente (2026-08-25), qui donnait `0142`/`0143` non déployées — elles le sont depuis. **Ne jamais présumer l'application d'une migration sans preuve** : `db push` réussi attesté par le porteur, ou lecture directe du tracker/catalogue sur la base réellement concernée (règle #4). **Règle acquise additionnelle (Lot D0-bis) : une affirmation de ce fichier n'est pas elle-même une preuve.** Cette attestation a une date de péremption implicite — tout lot qui en dépend pour une décision doit la revérifier contre le catalogue réel plutôt que la citer comme acquise, exactement comme le Lot D0 (diagnostic de provenance des commandes, `docs/diagnostics/F-D0-provenance-commandes.md`) a dû corriger des avertissements `[TABLE POSSIBLEMENT ABSENTE]` écrits sur la foi de la ligne précédente.
