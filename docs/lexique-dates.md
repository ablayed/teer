...
`[Non vérifié]` **Le `TZ` du runtime Node de production n'est pas fixé par le dépôt** (ni `vercel.json`, ni `next.config`, ni aucune variable) et n'est pas mesurable en lecture seule. Il est **présumé `UTC`**. Le Sénégal étant à UTC+0 toute l'année, sous cette présomption les bornes calculées côté serveur sont justes. **Si la présomption est fausse, toutes les bornes serveur du §4 se décalent d'un jour sur une partie de l'année.** C'est un réglage de plateforme, pas un commit.
...
## 4. Les trois régimes de fuseau, et le seul qui écrive en base

**a) Serveur, dans le `TZ` du process** — le régime de **tous** les écrans. `resolvePeriodRange` (`lib/periods/date-range.ts`:45-91) est la **seule** source des bornes de période : `today`/`yesterday`/`month` et les presets `7j`/`30j`/`90j` passent par `setHours(0,0,0,0)` / `setDate`, donc par le fuseau du process ; `custom` par `new Date("YYYY-MM-DDT00:00:00")`, minuit local du process. Appelée depuis 5 Server Components, 1 action serveur, **et 1 composant client** (voir c2).
...
- **c2 — `driver-cash-panel.tsx` appelle `resolvePeriodRange` côté client** (ligne 97). Le premier rendu de `/livreurs` utilise les bornes **du serveur** ; le `refreshCash()` qui suit l'enregistrement d'un versement les recalcule **dans le navigateur**. Sur un poste dont le fuseau diffère de celui du serveur, « Collecté sur période » **peut changer de valeur après un versement sans qu'aucune donnée de collecte n'ait bougé**. Non corrigé.

**L'affichage, lui, était déjà figé.** `lib/format/date.ts` (`DATE_TIME_ZONE`, ligne 1) rend toute date en `Africa/Dakar`. C'était la bonne pratique du dépôt, appliquée à l'affichage seulement ; `COHERENCE-01` l'a étendue à la **saisie**. Elle n'est **toujours pas** appliquée aux **bornes de lecture** (régime a).
...
| Charges, publicité | `expense.spent_at` / `product_ad_spend.spent_at` (`date`) | Correct, avec une réserve : le filtre est construit par troncature d'ISO (`from.toISOString().slice(0,10)`), donc **juste seulement si le process est en UTC** |
...
7. **`driver-cash-panel.tsx`** recalcule ses bornes côté client (§4-c2).
8. **Le `TZ` du runtime de production** n'est pas fixé par le dépôt, et **`test-unit` n'a pas de `TZ`** : il hérite de l'UTC des runners GitHub. La contrainte « le process doit être en UTC » est donc **encodée sans être exprimée**. Trois tests de `tests/unit/period-range.test.ts` ne passent que sous `TZ=UTC`.
