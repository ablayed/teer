...
| Expérience embarquée Shopify | ✅ code local | `shopify.app.toml` (`embedded = true`), `/shopify/embedded`, App Bridge en `<script>` bloquant (jamais `next/script`) |
...
| Anti-réimport après redaction | ⚠️ activé, preuve de régime réel à établir | Tombstones uniques `(merchant_account_id, shop_id, shopify_customer_id)`, expiration 12 mois, consultation avant sync normale/Bulk ; migration `0122` appliquée, cron activé `vercel.json` (`0 3 * * *`, `mode=execute`), volumétrie de contrôle nulle au moment de l'activation — preuve de purge en régime réel (arriéré non nul) encore à observer |
| Rétention temporelle PCD | ⚠️ activé, audit incomplet | 90 jours adresses après transition finale certaine, 12 mois identité après activité Shopify certaine, 7 jours payloads retryable ; preview SQL commun et purge bornée dans 0122 ; cron quotidien activé — **mais la purge automatique de l'identité client n'écrit aucun `audit_log`, contrairement au webhook `customers/redact` qui traite la même opération (voir docs/shopify-pcd-retention.md § Limites restantes) — à corriger avant soumission** |
| DSAR privé et borné | ⚠️ activé, preuve de régime réel à établir | Bucket Storage privé `shopify-dsar`, métadonnées sans PCD, contrôle serveur owner/manager et URL signée limitée à l'expiration avec maximum 24 h ; mécanismes `0122` appliqués, cron activé quotidiennement — preuve de purge en régime réel à établir |
...
- S1B-2B : la migration `0122` est appliquée en production et le cron est activé (`vercel.json`, `0 3 * * *`, `mode=execute`) ; une preuve de purge en régime réel (arriéré non nul) reste à observer — la validation d'activation portait sur une volumétrie de contrôle nulle.
- **Asymétrie de traçabilité GDPR, à traiter avant soumission Phase 4** : la purge automatique quotidienne de l'identité client (`execute_shopify_pcd_retention`, catégorie `expired_customer_identity`) appelle la même fonction de redaction que le webhook `customers/redact`, mais n'écrit aucune ligne `audit_log` — seul le webhook trace nommément cette destruction. Détail dans `docs/shopify-pcd-retention.md` § Limites restantes.
