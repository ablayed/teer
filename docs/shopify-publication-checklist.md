...
| Anti-réimport après redaction | ⚠️ code local | Tombstones uniques `(merchant_account_id, shop_id, shopify_customer_id)`, expiration 12 mois, consultation avant sync normale/Bulk ; purge temporelle S1B-2B |
| DSAR privé et borné | ⚠️ code local | Bucket Storage privé `shopify-dsar`, métadonnées sans PCD, contrôle serveur owner/manager et URL signée limitée à l'expiration avec maximum 24 h ; suppression des artefacts expirés S1B-2B |
...
3. Demander l'accès aux **données client protégées** (protected customer data) et justifier l'usage COD. Ne déclarer que les champs réellement traités : nom/prénom, téléphone et adresse de livraison. Le téléphone est l'identité principale pour la déduplication et les appels de confirmation ; le nom et l'adresse sont nécessaires à l'exécution de la livraison.
   ...
   - Justifier la **minimisation** : conservation limitée aux données nécessaires à la livraison COD. S1B-2A couvre la redaction transactionnelle, les tombstones anti-réimport de 12 mois et les artefacts DSAR expirant après 24 heures ; la migration n'est pas encore une preuve de production.
   - **S1A appliqué :** l'ingestion normale, bulk et webhook ne demande ni ne stocke `tags`, `numberOfOrders`, `amountSpent`, le `createdAt` du client Shopify ou le consentement marketing. Les données conservées sont le nom/prénom, le téléphone, l'adresse et les identifiants techniques Shopify strictement nécessaires ; `orders.shipping_address` reste une copie opérationnelle pour la livraison. La rétention générale, le cron de purge, les erreurs GDPR âgées de plus de 7 jours, le chiffrement de production, la DLP, la journalisation générale des lectures PCD, les sauvegardes et la réponse aux incidents restent à traiter dans S1B-2B, S1C et S1D.
...
- S1B-2B : purge des payloads retryable âgés de plus de 7 jours, purge des tombstones expirés, suppression des artefacts DSAR expirés et déclenchement planifié ; aucun cron n'est installé par S1B-2A.
