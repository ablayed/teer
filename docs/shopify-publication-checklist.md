...
| 3 webhooks GDPR **réels** sur PII réelle (7b) | ✅ | `customers/data_request` (compile les champs conservés), `customers/redact` (efface nom/prénoms/téléphone/phone_e164/adresse, retrouvé par GID legacy **et** tableau `shopify_customer_gids`), `shop/redact` (clients **exclusifs** à la boutique) → `lib/shopify/gdpr.ts` |
...
3. Demander l'accès aux **données client protégées** (protected customer data) et justifier l'usage COD. Ne déclarer que les champs réellement traités : nom/prénom, téléphone et adresse de livraison. Le téléphone est l'identité principale pour la déduplication et les appels de confirmation ; le nom et l'adresse sont nécessaires à l'exécution de la livraison.
   ...
   - Justifier la **minimisation** : conservation limitée aux données nécessaires à la livraison COD ; les délais de rétention et de purge générale ne sont pas couverts par S1A et ne doivent pas être déclarés avant S1B. Les webhooks `customers/redact` / `shop/redact` restent les voies d'effacement Shopify concernées.
   - **S1A appliqué :** l'ingestion normale, bulk et webhook ne demande ni ne stocke `tags`, `numberOfOrders`, `amountSpent`, le `createdAt` du client Shopify ou le consentement marketing. Les données conservées sont le nom/prénom, le téléphone, l'adresse et les identifiants techniques Shopify strictement nécessaires ; `orders.shipping_address` reste une copie opérationnelle pour la livraison. La rétention générale, les contrôles d'accès et les sauvegardes restent à traiter dans les lots S1B–S1D.
