-- ============================================================
-- 0049 : Retrait de l'email du CLIENT FINAL (minimisation COD)
-- ============================================================
-- Décision produit (PCD) : Tëër ne demande PAS l'accès à l'email du
-- client final Shopify (non utilisé en COD ; le demander affaiblit la
-- minimisation). La CAPACITÉ même de stocker cet email doit disparaître,
-- pas seulement être vidée.
--
-- ⚠️ Email concerné = celui du CLIENT FINAL (table public.customer).
--    L'email du MARCHAND (auth.users / merchant_member / invitation) n'est
--    PAS touché : il reste traité pour le compte.
--
-- Ce que fait cette migration :
--   1. Recrée les 2 RPC de fiabilité sans la colonne email
--      (DROP obligatoire : on ne peut pas changer la signature RETURNS TABLE
--       d'une fonction existante via CREATE OR REPLACE).
--   2. Supprime la colonne public.customer.email — le DROP détruit les
--      valeurs stockées : c'est la purge.
--
-- Ordre impératif : recréer les fonctions AVANT de dropper la colonne, sinon
-- le DROP COLUMN échoue (dépendance des fonctions SQL inlinées sur c.email).
-- list_customer_reliability dépend de get_customer_reliability → drop list d'abord.
--
-- Corps des fonctions = copie EXACTE de 0014 (logique de scoring inchangée),
-- seul le champ `email` (RETURNS TABLE + SELECT) est retiré.
--
-- Après push : pnpm db:types (la colonne customer.email et le champ email des
-- 2 RPC disparaissent des types générés ; plus aucun code ne les référence).
-- ============================================================

drop function if exists public.list_customer_reliability(uuid, text, integer, integer, boolean);
drop function if exists public.get_customer_reliability(uuid, uuid);

create function public.get_customer_reliability(
  p_merchant_id uuid,
  p_customer_id uuid
)
returns table (
...
create function public.list_customer_reliability(
  p_merchant_id uuid,
...
-- Purge + suppression de la capacité de stockage : le DROP détruit les valeurs.
alter table public.customer drop column if exists email;
