-- ============================================================
-- 0031 : phase4 — stock en main du livreur
-- ============================================================
-- 1. stock_movement.driver_id (nullable) : attribue un mouvement
--    à un livreur (mode lot hors commande ET mode par-commande).
-- 2. Deux movement_type lot : allocate_to_courier (sortie entrepôt
--    vers le livreur, hors commande) / courier_return_lot (retour
--    de l'invendu). CHECK recréé NOT VALID (n'invalide pas l'existant).
-- 3. post_stock_movement étendu : accepte p_driver_id et traite les
--    deux nouveaux types (allocate décrémente qty_on_hand entrepôt +
--    snapshot CUMP ; courier_return_lot le restaure).
-- 4. transition_order attribue assigned_driver_id à chaque mouvement
--    de commande (dispatch, sold, …) via p_driver_id.
--
-- Le « stock en main du livreur » reste DÉRIVÉ du ledger
-- (Σ allocate_to_courier + dispatch − sold − courier_return_lot),
-- jamais une source de vérité dupliquée. L'invariant tient :
-- une sortie (dispatch/allocate) décrémente qty_on_hand entrepôt.
-- ============================================================

-- ────────────────────────────────────────────────────────────
-- 1. Colonne driver_id (nullable, ledger append-only)
-- ────────────────────────────────────────────────────────────
...
create index if not exists stock_movement_driver_idx
  on public.stock_movement (driver_id)
  where driver_id is not null;

-- driver_id lisible (comme les autres colonnes ; unit_cost reste hors grant).
grant select (driver_id) on public.stock_movement to authenticated;

-- ────────────────────────────────────────────────────────────
-- 2. Nouveaux movement_type lot (CHECK recréé NOT VALID)
-- ────────────────────────────────────────────────────────────
...
-- Les mouvements lot exigent un livreur ; les autres restent libres.
alter table public.stock_movement
  add constraint stock_movement_lot_requires_driver_check
  check (
    movement_type not in ('allocate_to_courier', 'courier_return_lot')
    or driver_id is not null
  )
  not valid;

-- ────────────────────────────────────────────────────────────
-- 3. post_stock_movement : + p_driver_id + types lot
-- ────────────────────────────────────────────────────────────
-- L'ajout d'un paramètre crée une signature distincte : on droppe
-- l'ancienne pour éviter toute ambiguïté d'overload.

drop function if exists public.post_stock_movement(
  uuid, uuid, text, integer, text, uuid, uuid, uuid, bigint, text
);
...
-- ────────────────────────────────────────────────────────────
-- 4. transition_order : attribue le livreur au mouvement de commande
-- ────────────────────────────────────────────────────────────
-- Seule différence avec 0029 : on calcule le livreur effectif
-- (coalesce(p_assigned_driver_id, v_order.assigned_driver_id)) et on
-- le passe à post_stock_movement (p_driver_id) pour que dispatch/sold/
-- release portent le livreur — dérivation stock en main par un seul
-- group by driver_id.
