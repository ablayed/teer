-- 0106 — « Refuser » → « Reprogrammer » en cours de livraison (Phase B).
--
-- Nouvelle action `reprogrammer` (EN_LIVRAISON → PROGRAMMEE, lib/domain/order-
-- transition-actions.ts) : report logistique, pas un échec de livraison. Son patch 4D
-- pose delivery_state='scheduled' + clear_assigned_driver=true tout en gardant
-- order_state='open' (jamais 'cancelled'/'returned').
--
-- La garde existante de `order_assignment_release` (migration 0091, inchangée depuis)
-- est PILOTÉE PAR order_state (`v_next_order_state in ('cancelled','returned')`) — elle
-- ne se déclenche donc jamais pour reprogrammer, qui garde order_state='open'. Décision
-- Phase B actée : reprogrammer DOIT libérer le stock engagé côté livreur (pas de
-- réservation conservée en attendant la nouvelle tentative). Ajout d'une seconde
-- condition, combinée par OR, déclenchée par le delta delivery_state
-- assigned/out_for_delivery → scheduled — signature comportementale unique de
-- reprogrammer (aucune autre action ne produit ce delta précis). RÉUTILISE la MÊME
-- boucle de release, inchangée : ciblage par ledger (`open_commitments`, jamais par
-- `assigned_driver_id` seul), plafonnée à `least(required_qty, net_open)` par
-- produit+livreur — aucune sur-libération possible, même garde que Lot 2.
--
-- p_clear_assigned_driver=true (déjà posé par le patch TS de reprogrammer) déclenche en
-- plus, sans changement ici, le bloc pré-existant (lignes finales de la fonction) qui
-- reverse les mouvements advance_commit ouverts pour cette commande — complète la
-- libération pour les commandes couvertes par un lot d'avance plutôt qu'un dispatch.
--
-- Périmètre du changement : UNE SEULE condition (le `if` qui déclenche la boucle
-- order_assignment_release). Corps de la fonction repris VERBATIM de la dernière
-- version vivante (migration 0096 — confirmé : aucune migration entre 0096 et 0105 ne
-- touche transition_order) pour tout le reste. Signature identique (aucun nouveau
-- paramètre, reprogrammer réutilise p_delivery_state/p_call_state/p_cash_state/
-- p_scheduled_for/p_clear_assigned_driver déjà existants) — pas de nouveau grant/revoke
-- nécessaire (même convention que 0096/0086).
--
-- Hors scope, non touché : `annuler`/`refuser` (garde de release cancelled/returned
-- inchangée à l'identique), cash_collected_at (fix 0096 inchangé), tout autre mouvement
-- (dispatch, sold, courier_return, reserve/release entrepôt), returned_at.
...
begin
  ...
  -- 0106 : deuxième déclencheur (OR) — reprogrammer garde order_state='open' (jamais
  -- cancelled/returned), donc le release doit aussi se déclencher sur le delta
  -- delivery_state assigned/out_for_delivery → scheduled, signature unique de
  -- reprogrammer. Boucle de release elle-même INCHANGÉE.
  if (
