-- ============================================================
-- 0059 : phase11 — la reserve molle se pose aussi sur le chemin « scheduled »
-- ============================================================
-- Contexte : Phase 11 fusionne confirmation + programmation en une seule
-- transition « Programmer » depuis « A appeler » (delivery unassigned -> scheduled,
-- call to_call/callback -> validated EN UNE FOIS). La branche `reserve` de
-- transition_order ne se declenchait que si l'etat d'arrivee delivery etait
-- 'unassigned' -> donc SAUTEE sur le chemin fusionne.
--
-- Bug : le `dispatch` fait -qty_reserved (il LIBERE la reserve molle posee a la
-- validation). Si la reserve (+qty_reserved) n'a jamais ete posee, le -qty_reserved
-- du dispatch passe qty_reserved en negatif (ou decremente a tort la reserve
-- d'AUTRES commandes du meme produit, la projection product_stock etant partagee).
-- Comme « Programmer » remplace « Confirmer » comme chemin principal, la reserve
-- molle devenait quasi non-fonctionnelle (sous-comptee) sur toutes les commandes
-- programmees en direct.
--
-- Fix chirurgical : la branche `reserve` se declenche sur call -> validated quand
-- l'etat d'arrivee delivery est PRE-DISPATCH, c.-a-d. in ('unassigned','scheduled').
-- La garde `v_order.call_state <> 'validated'` est CONSERVEE : un seul `reserve`
-- par commande (un passage unassigned->scheduled separe, call deja validated, ne
-- re-reserve pas). Symetrique de la branche `release` (deconfirmer) qui couvre
-- deja {unassigned, scheduled}.
--
-- Le RESTE de transition_order est reproduit BYTE-FOR-BYTE depuis 0056 (Lot D) :
-- aucun autre comportement (mark_returned, reprise cash, horodatages 0035, fixes
-- 0056/0057) n'est touche. Seule la condition de la branche `reserve` change.
-- ============================================================
