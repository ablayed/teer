-- ============================================================
-- 0062 : phase11.1 — « Assignée » reste dans « Programmer », hors En-livraison
-- ============================================================
-- Correction du flux d'assignation (option C, retour marchand) : une commande
-- assignée (livreur choisi) mais dont le popup détails/prix n'est pas encore
-- confirmé NE DOIT PAS apparaître dans « En cours de livraison ». Elle n'y passe
-- qu'APRÈS confirmation du popup (delivery_state=out_for_delivery).
--
-- Donc on déplace `assigned` de la vue « En cours de livraison » vers la vue
-- « Programmer » (id `confirmee`), où elle reste récupérable (réouverture du popup).
--
--   Programmer (confirmee) : open + validated + delivery ∈ {unassigned, scheduled, assigned}
--   En cours de livraison   : delivery = out_for_delivery (seul)
--
-- Filtre de vue en SQL → on modifie les DEUX cas (confirmee + en-livraison) dans
-- list_orders_paginated (def. courante 0061) ET orders_view_counts (def. courante
-- 0061). Type de retour inchangé ⇒ `create or replace` (pas de DROP). Corps
-- reproduits À L'IDENTIQUE, seuls les deux prédicats changent.
-- ============================================================

-- 1. list_orders_paginated (def. courante = 0061 ; type de retour inchangé)
...
-- 2. orders_view_counts (def. courante = 0061 ; type de retour inchangé)
  ...
  select
    ...
    count(*) filter (
      where order_state = 'open'
        ...
        and delivery_state in ('unassigned', 'scheduled', 'assigned')
    ) as confirmee,
    count(*) filter (
      where delivery_state = 'out_for_delivery'
    ) as en_livraison,
    count(*) filter (
